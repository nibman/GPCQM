...
Athletes =
{
    keys:["objectId", "firstName", "lastName", "athleteId", "dossard", "country", "dob", "hrm", "srmPowerMeter",
            "vectorPowerMeter", "quarqPowerMeter", "speedCadence", "powerMeter",
            "garminSpeedCadence", "team", "gps", "wheelCir", "showData"],

    resultModel:["firstName", "lastName", "athleteId", "dossard", "country", "dob", "hrm", "srmPowerMeter",
            "powerMeter", "vectorPowerMeter", "quarqPowerMeter", "speedCadence",
            "garminSpeedCadence", "team", "gps", "objectId", "showData"],
    ...
    saveAthlete:function(athleteID, athleteObj, successCB, err)
    {
        if (athleteID != null)
        {
            var Athlete = Parse.Object.extend("Athlete");
            var query = new Parse.Query(Athlete);
            query.equalTo("athleteId", athleteID);
            ...
            query.find(
            {
                success:function(object)
                {
                    ...
                    athleteObj.id = object[0].id;
                    athleteObj.objectId = object[0].id;

                    athlete.set("objectId", object[0].id);
                    athlete.set("id", object[0].id);
                    athlete.save(null, {
                        success: function(athlete) {
                            for (key in athleteObj) {
                                if (key != "id" && key != "objectId" && key != "athleteId" && key != "showData" && athleteObj[key]) {
                                  console.log( " Key : " + key + "= = = athleteObj[key] : " + athleteObj[key]);
                                    athlete.set(key, athleteObj[key]);
                                }

                                if ("dob" == key) {
                                    date = athleteObj[key].split("-")
                                    date = new Date(date[0], parseInt(date[1])-1, parseInt(date[2]))
                                    athlete.set(key, date);
                                }
                            }
    ...
    saveAthlete_old:function(athleteID, athleteObj, successCB, err)
    {
        if (athleteID != null)
        {
            var Athlete = Parse.Object.extend("Athlete");
    ...
    createAthlete:function(athleteObj, successCB, err)
    {
        ...
        for(var e in athleteObj)
        {
            console.log(e + ' - - ' + athleteObj[e]);
            if (e != "objectId")
            {
                if ("dob" == e) {
                    date = athleteObj[e].split("-")
                    date = new Date(date[0], parseInt(date[1])-1, parseInt(date[2]))
                    athleteObj[e] = date;
                }
                console.log(e + ' - - ' + athleteObj[e]);
                a.set(e, (athleteObj[e] == "on" ? true : athleteObj[e]));
            }
        }
    ...
    getAllTeams:function()
    {
        ...
        query.find(
        {
            success:function(object)
            {
                _select = document.getElementById("team");

                for (var i = 0; i < object.length; i++) {
                    var option = document.createElement("option");
                    option.value = object[i].get('teamName');
                    option.text = object[i].get('teamName');
...
// Athletes.setKeyNames("gpcqm1", ["firstName", "lastName", "athleteId", "dossard", "country", "dob", "hrm", "pm", "sc", "team", "gps","objectId", "showData"],function(e){});
...
{
    _fields = document.querySelector('.active').getElementsByTagName('td')

    for (i in _fields ) {
        if (i < 12 ) {

            _field = _fields[i];
            _fldName = _field.getAttribute("data-field");
            _fldvalue =  _field.innerText;

            // Date string manipulation.
            if (_fldName == "dob") {
                 _data = _fldvalue;
                _data = _data.replace(/^\w{3} /, '');
                _data = _data.replace(/ 00:.*/, '');
               _arrMonth = {jan:"01", fev:"02", mar:"03", avr:"04", mai:"05", jun:"06", jul:"07", aug:"08", sep:"09", oct:"10", nov:"11", dec:"12" };
               _arr = _data.split(" ");
               mth = _arr[0].toLowerCase();
               console.log( _arr.length );
               console.log( mth );
               console.log( _arrMonth[mth] );
               if (_arr.length == 3)
                _fldvalue = _arr[2] + "-" + _arrMonth[mth] + "-" + _arr[1];
            }
            console.log(i +  " Field : " + _fldName + " == == Value : " + _fldvalue )
            _iptField = document.getElementById(_fldName);
            if (_iptField != null)
                console.log( _iptField.value = _fldvalue );
        }
    }
...
function athletesReceived(a)
{
 ...
 {
     _athleteId = parseInt(a[i][2]);
     _id = _athleteId > _id ? _athleteId : _id;
    ...
    for (j = 0; j <a[i].length; j++)
    {
      ...
      td.appendChild(document.createTextNode(content))
              td.setAttribute("data-field", keyNames[j]);
      tr.appendChild(td);
    }
    dynamicTableBody.appendChild(tr);
 }

 Athletes.getAllTeams();

 document.getElementById("athleteId").value = _id + 1;
...
$(document).on('confirmation', '.remodal', function () {
  if (checkform(form))
  {
    ...
    for(var i=0; i<formElem.length; i++)
    {
      ...
      if ("athleteId" == formElem[i].name) {
          _athleteId = formElem[i].value;
      }
    }

    if (_athleteId && _athleteId > 0)
        Athletes.saveAthlete(_athleteId, fields, athleteCreated, errorReceived);
    else
